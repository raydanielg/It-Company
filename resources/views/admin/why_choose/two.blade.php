...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">{{ __('Why Choose Two - Items') }}</h1>
</div>
...
<div class="row">
    <div class="col-md-7">
        <div class="card shadow mb-4">
            <div class="card-body">
                <form action="{{ route('admin_why_choose_two_item_update') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <label for="" class="form-label">{{ __('Heading') }}</label>
                        <input type="text" name="heading" class="form-control" value="{{ $why_choose_two_items->heading }}">
                    </div>
                    <div class="mb-3">
                        <label for="" class="form-label">{{ __('Subheading') }}</label>
                        <input type="text" name="subheading" class="form-control" value="{{ $why_choose_two_items->subheading }}">
                    </div>
                    <div class="mb-3">
                        <label for="" class="form-label">{{ __('Photo Over Text') }}</label>
                        <textarea name="photo_over_text" class="form-control h_100" cols="30" rows="10">{{ $why_choose_two_items->photo_over_text }}</textarea>
                    </div>
                    <div class="mb-3">
                        <label for="" class="form-label">{{ __('Photo Over heading') }}</label>
                        <input type="text" name="photo_over_heading" class="form-control" value="{{ $why_choose_two_items->photo_over_heading }}">
                    </div>
                    <div class="row">
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <div class="photo-container">
                                    <a href="{{ asset('uploads/'.$why_choose_two_items->photo) }}" class="magnific"><img src="{{ asset('uploads/'.$why_choose_two_items->photo) }}" alt=""></a>
                                </div>
                            </div>
                            <div class="mb-3">
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form action="{{ route('admin_why_choose_two_item_element_submit') }}" method="post">
                                    @csrf
                                    <div class="mb-3">
                                        <label for="" class="form-label">{{ __('Heading') }}*</label>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('admin_why_choose_two_item_element_delete',$item->id) }}" class="btn btn-danger btn-sm btn-block" onClick="return confirm('{{ __('Are you sure?') }}')"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                            ...
                            <div class="modal fade" id="edit_modal_{{ $loop->iteration }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form action="{{ route('admin_why_choose_two_item_element_update',$item->id) }}" method="post">
                                                @csrf
                                                <div class="mb-3">
                                                    <label for="" class="form-label">{{ __('Heading') }}*</label>
